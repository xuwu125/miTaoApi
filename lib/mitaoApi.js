...
if (window.mitaoApi == undefined) {
    ...
    window.mitaoApi = {
        ...
        invoke: function(apiname, param, callbackfun) {
            try {
                bridge.registerHandler(apiname, callbackfun)
                window.WebViewJavascriptBridge.callHandler(apiname, JSON.stringify(param),function(responseData){
                    return responseData
                });
                // 先处理回调
